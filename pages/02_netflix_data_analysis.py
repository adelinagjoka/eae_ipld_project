...
top_10_countries = (
    movies_df[movies_df['release_year'] == year]  # filter by year
    .groupby('country')['title']                  # group by country
    .count()                                      # count movies/series
...
movies_only = movies_df[movies_df['type'] == 'Movie'] # First, filter only movies
# Extract numeric duration in minutes
# Some durations may be strings like "90 min", so we need to clean them
movies_only['duration_minutes'] = movies_only['duration'].str.replace(" min", "").astype(float)

# Group by release_year and calculate mean
